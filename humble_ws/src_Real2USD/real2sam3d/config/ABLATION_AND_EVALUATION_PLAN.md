...
- **Ablation runner** (optional): Script that, for a list of run dirs and config tags, (1) finds the right prediction file (USDA or exported scene_graph → evaluation JSON), (2) calls `run_eval.py`, (3) collects metrics into a single table (CSV + optional Markdown/LaTeX). See §4 for output formats.
