...
class ColorPCNode(Node):
    def __init__(self):
        ...
        self.create_subscription(PointCloud2, "/point_cloud2", self.lidar_callback, 10)
        self.pub = self.create_publisher(PointCloud2, "/point_cloud2_accumulated", 10)
        ...
        # Parameters
        self.declare_parameter('save_interval', 30)  # Save every 10 frames
        self.declare_parameter('output_dir', 'evaluations/pc_data')  # Directory to save PLY files
        ...
        self.output_file = f"{self.get_parameter('output_dir').value}/{date}_voxel_pointcloud.ply"
    ...
    def lidar_callback(self, msg: PointCloud2):
        ...
        if self.frame_count >= self.get_parameter('save_interval').value:
            self.save_to_ply()
            self.frame_count = 0
            # publish the accumulated points
            accum_msg = point_cloud2.create_cloud_xyz32(header=Header(frame_id="odom"), points=self.accumulated_points)
            accum_msg.header.stamp = self.get_clock().now().to_msg()
            self.pub.publish(accum_msg)
def main():
    rclpy.init()
